...
function GeneratePanel({ credits, setCredits, fetchHistory, darkMode, inputBg, textSecondary, textMuted, cardBg, textPrimary }: any) {
  ...
  const handleExportHTML = () => { if (!output) return; const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Content</title><style>body{font-family:system-ui;max-width:700px;margin:2rem auto;padding:0 1rem;line-height:1.7;}</style>
    </head><body>${output.replace(/\n/g, "<br>")}</body></html>`; const blob = new Blob([html], { type: "text/html" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "content.html"; a.click(); };
        ...
        <Card className={cardBg}>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-2"><Label className={textSecondary}>Tone</Label><div className="relative"><button type="button" onClick={() => { setShowToneDropdown(!showToneDropdown); setShowLangDropdown(false); setShowLengthDropdown(false); }} className={`w-full flex items-center justify-between rounded-md border px-3 py-2 text-sm ${inputBg}`}><span className="capitalize">{tone}</span>
                ...
                {TONES.map(t => (<button key={t} type="button" onClick={() => { setTone(t); setShowToneDropdown(false); }} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-emerald-500/10 capitalize ${tone === t ? "text-emerald-500 font-medium" : textSecondary}`}>{t}</button>))}</motion.div>)}</AnimatePresence></div></div>
              <div className="space-y-2"><Label className={textSecondary}>Length</Label><div className="relative"><button type="button" onClick={() => { setShowLengthDropdown(!showLengthDropdown); setShowToneDropdown(false); setShowLangDropdown(false); }} className={`w-full flex items-center justify-between rounded-md border px-3 py-2 text-sm ${inputBg}`}><span className="capitalize">{length}</span>
                <ChevronDown className="w-3 h-3" /></button><AnimatePresence>{showLengthDropdown && (<motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} className={`absolute top-full left-0 right-0 mt-1 border rounded-md shadow-lg z-10 overflow-hidden ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
                {LENGTHS.map(l => (<button key={l} type="button" onClick={() => { setLength(l); setShowLengthDropdown(false); }} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-emerald-500/10 capitalize ${length === l ? "text-emerald-500 font-medium" : textSecondary}`}>{l}</button>))}</motion.div>)}</AnimatePresence></div></div>
              <div className="space-y-2"><Label className={textSecondary}>Language</Label><div className="relative"><button type="button" onClick={() => { setShowLangDropdown(!showLangDropdown); setShowToneDropdown(false); setShowLengthDropdown(false); }} className={`w-full flex items-center justify-between rounded-md border px-3 py-2 text-sm ${inputBg}`}><span>{LANGUAGES.find(l => l.code === language)?.label || "English"}</span>
                ...
                {LANGUAGES.map(l => (<button key={l.code} type="button" onClick={() => { setLanguage(l.code); setShowLangDropdown(false); }} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-emerald-500/10 ${language === l.code ? "text-emerald-500 font-medium" : textSecondary}`}>{l.label}</button>))}</motion.div>)}</AnimatePresence></div></div>
            </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between"><CardTitle className="text-lg flex items-center gap-2"><FileText className="w-5 h-5 text-emerald-500" />Output</CardTitle>
              {output && (<div className="flex gap-1"><Button variant="ghost" size="sm" onClick={handleCopy}>{copied ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}<span className="ml-1 text-xs">{copied ? "Copied!" : "Copy"}</span></Button><Button variant="ghost" size="sm" onClick={handleExportText}><Download className="w-3.5 h-3.5" /><span className="ml-1 text-xs">.txt</span></Button>
                <Button variant="ghost" size="sm" onClick={handleExportHTML}><Download className="w-3.5 h-3.5" /><span className="ml-1 text-xs">.html</span></Button><Button variant="ghost" size="sm" onClick={() => { setOutput(""); setTopic(""); setSelectedType(""); }}><RotateCcw className="w-3.5 h-3.5" /></Button></div>)}
            </div>
          </CardHeader>
          <CardContent>
            {generating ? (<div className="flex flex-col items-center justify-center py-16 text-gray-400"><Loader2 className="w-8 h-8 animate-spin text-emerald-500 mb-4" /><p className="text-sm">Creating your content...</p></div>)
              : output ? (<ScrollArea className="h-[400px]"><div className={`prose prose-sm max-w-none whitespace-pre-wrap ${darkMode ? "text-gray-300" : "text-gray-700"} leading-relaxed`}>{output}</div>
              </ScrollArea>) : (<div className="flex flex-col items-center justify-center py-16 text-gray-400"><div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 ${darkMode ? "bg-gray-800" : "bg-gray-100"}`}><Sparkles className={`w-8 h-8 ${textMuted}`} /></div>
              <p className={`text-sm font-medium ${textSecondary}`}>Your content will appear here</p><p className={`text-xs ${textMuted} mt-1`}>Select a type, topic, language, and generate</p></div>)}
          </CardContent>
        </Card>
      </div>
    </div>
    ...
    <div className="max-w-3xl mx-auto">
      <Card className={cardBg}>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2"><Label className={textSecondary}>Repurpose for</Label><div className="flex flex-wrap gap-2">{PLATFORMS.slice(0, 6).map(p => (<button key={p} onClick={() => setTargetPlatform(p.toLowerCase())} className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all capitalize ${targetPlatform === p.toLowerCase() ? "bg-emerald-500 text-black border-emerald-500" : cardBg}`}>{p}</button>))}</div>
            </div>
          ...
          {output && (<div className={`mt-4 p-4 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-800/50" : "border-gray-200 bg-gray-50"}`}><div className="flex justify-between mb-2"><p className="text-sm font-medium capitalize">For {targetPlatform}</p><Button variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(output); setCopied(true); setTimeout(() => setCopied(false), 2000); }}>{copied ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}</Button></div>
            <p className={`text-sm whitespace-pre-wrap ${darkMode ? "text-gray-300" : "text-gray-700"}`}>{output}</p></div>)}
        </CardContent>
      </Card>
    </div>
  );
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <Button variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(hashtags.join(" ")); setCopied(true); setTimeout(() => setCopied(false), 2000); }}>
                  ...
                  <span className="ml-1 text-xs">{copied ? "Copied!" : "Copy All"}</span>
                </Button>
              </div>
...
// ── Calendar Panel ──
function CalendarPanel({ darkMode, cardBg, textSecondary, inputBg, textMuted }: any) {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <Card className={cardBg}><CardHeader><CardTitle className="text-sm">Schedule Post</CardTitle></CardHeader><CardContent className="space-y-3"><Input placeholder="Post title" value={title} onChange={e =>
            setTitle(e.target.value)} className={inputBg} /><input type="date" value={date} onChange={e => setDate(e.target.value)} className={`w-full rounded-md border px-3 py-2 text-sm ${inputBg}`} /><div className="flex flex-wrap gap-1">{PLATFORMS.slice(0, 5).map(p => (<button key={p} onClick={() => setPlatform(p)} className={`px-2 py-1 rounded text-xs border transition-all ${platform === p ? "bg-emerald-500 text-black border-emerald-500" : cardBg}`}>{p}</button>))}</div>
            <Button onClick={handleAdd} disabled={!title || !date} className="w-full bg-emerald-500 hover:bg-emerald-400 text-black text-sm h-9"><Plus className="w-3.5 h-3.5 mr-1" />Schedule</Button></CardContent></Card>
          <Card className={cardBg}><CardHeader><CardTitle className="text-sm">Upcoming</CardTitle></CardHeader><CardContent className="space-y-2 max-h-64 overflow-y-auto">{events.length === 0 ? <p className={`text-xs ${textMuted}`}>No scheduled posts</p> : events.slice(0, 10).map(e => (<div key={e.id} className={`flex items-center justify-between p-2 rounded-lg ${darkMode ? "bg-gray-800/50" : "bg-gray-50"}`}><div><p className={`text-xs font-medium ${textSecondary}`}>{e.title}</p>
            <p className={`text-[10px] ${textMuted}`}>{e.date} · {e.platform}</p></div><button onClick={() => handleDelete(e.id)} className="text-red-400 hover:text-red-300"><Trash2 className="w-3 h-3" /></button></div>))}</CardContent></Card>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {templates.map(t => (<Card key={t.id} className={`cursor-pointer transition-all hover:border-emerald-500/30 ${selected?.id === t.id ? "border-emerald-500" : cardBg}`} onClick={() => setSelected(t)}><CardContent className="p-4"><div className="flex items-center justify-between mb-2"><Badge variant="secondary" className="text-[10px] capitalize">{t.type}</Badge>{t.isPremium && <Badge className="text-[10px] bg-amber-500 text-black">PRO</Badge>}</div>
          <p className={`text-sm font-medium mb-1 ${textSecondary}`}>{t.title}</p><p className={`text-xs ${textMuted} line-clamp-2`}>{t.prompt}</p></CardContent></Card>))}
      </div>
      {selected && (<div className={`fixed bottom-0 left-0 right-0 p-4 border-t ${darkMode ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200"}`}><div className="max-w-4xl mx-auto flex items-center gap-3"><div className="flex-1"><Input placeholder="Enter your topic..." value={topic} onChange={e => setTopic(e.target.value)} className={inputBg} /></div>
        <Button onClick={handleUse} disabled={!topic.trim()} className="bg-emerald-500 hover:bg-emerald-400 text-black"><Sparkles className="w-4 h-4 mr-2" />Use Template</Button><Button variant="ghost" onClick={() => setSelected(null)}>✕</Button></div></div>)}
    </div>
  );
}
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <Card className={cardBg}><CardHeader><CardTitle className="text-lg flex items-center gap-2"><Mic className="w-5 h-5 text-emerald-500" />Brand Voice</CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-3"><div className="space-y-1"><Label className={textSecondary}>Voice Name</Label><Input placeholder="e.g., Professional Corporate" value={name} onChange={e => setName(e.target.value)} className={inputBg} /></div>
          <div className="space-y-1"><Label className={textSecondary}>Tone</Label><Input placeholder="e.g., Authoritative yet friendly" value={tone} onChange={e => setTone(e.target.value)} className={inputBg} /></div>
          <div className="space-y-1"><Label className={textSecondary}>Writing Style</Label><Input placeholder="e.g., Clear, concise, data-driven" value={style} onChange={e => setStyle(e.target.value)} className={inputBg} /></div>
          <div className="space-y-1"><Label className={textSecondary}>Target Audience</Label><Input placeholder="e.g., Tech-savvy millennials" value={audience} onChange={e => setAudience(e.target.value)} className={inputBg} /></div></div>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        ].map(s => (<Card key={s.label} className={cardBg}><CardContent className="p-4"><div className="flex items-center justify-between"><div><p className={`text-xs ${textSecondary}`}>{s.label}</p><p className="text-2xl font-bold mt-1">{s.value}</p></div>
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${darkMode ? "bg-emerald-500/10" : "bg-emerald-50"}`}><s.icon className="w-5 h-5 text-emerald-500" /></div></div></CardContent></Card>))}
      </div>
      <div className="grid sm:grid-cols-2 gap-4">
        <Card className={cardBg}><CardHeader><CardTitle className="text-sm">By Content Type</CardTitle></CardHeader><CardContent><div className="space-y-3">{stats?.byType ? Object.entries(stats.byType).map(([type, count]: any) => (<div key={type} className="flex items-center justify-between"><span className={`text-sm capitalize ${textSecondary}`}>{type}</span>
          <div className="flex items-center gap-2"><div className={`h-2 rounded-full ${darkMode ? "bg-gray-700" : "bg-gray-200"}`} style={{ width: "100px" }}><div className="h-2 rounded-full bg-emerald-500" style={{ width: `${Math.min(100, (count / stats.total) * 100)}%` }} /></div>
          <span className="text-xs font-medium w-6 text-right">{count}</span></div></div>)) : <p className={`text-sm ${textSecondary}`}>No data yet</p>}</div></CardContent></Card>
        <Card className={cardBg}><CardHeader><CardTitle className="text-sm">Activity (Last 7 Days)</CardTitle></CardHeader><CardContent><div className="space-y-2">{stats?.byDay ? Object.entries(stats.byDay).slice(-7).map(([day, count]: any) => (<div key={day} className="flex items-center justify-between"><span className={`text-xs ${textSecondary}`}>{day}</span><div className="flex items-center gap-1">{Array.from({ length: Math.min(20, count as number) }, (_, i) => (<div key={i} className="w-1.5 h-4 rounded-full bg-emerald-500" />))}</div>
          </div>)) : <p className={`text-sm ${textSecondary}`}>No data yet</p>}</div></CardContent></Card>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <Card className={cardBg}><CardHeader><CardTitle className="text-lg flex items-center gap-2"><Users className="w-5 h-5 text-emerald-500" />Team Workspaces</CardTitle></CardHeader><CardContent className="space-y-4">
        <p className={`text-sm ${textSecondary}`}>"Invite team members to collaborate. You can invite anyone regardless of plan."</p>
